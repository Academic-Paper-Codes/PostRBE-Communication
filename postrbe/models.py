...
class OpCount:
    ...
    setup_muladds: int
    keygen_muladds: int
    register_validation_muladds: int
    register_update_muladds: int
...
def opcount_postrbe(params: RBEParams) -> OpCount:
    ...
    keygen = p.n * p.t * p.t
    register_validation = (B - 1) * p.n * p.m * p.t
    register_update = (B - 1) * p.m * p.t + p.n * p.t
    register = register_validation + register_update
    ...
    return OpCount(
        ...
        register_validation_muladds=register_validation,
        register_update_muladds=register_update,
...
def opcount_postrbe_star(params: RBEParams) -> OpCount:
    ...
    keygen = p.n * p.t * p.t
    register_validation = p.n * p.r * p.t
    register_update = (B - 1) * p.m * p.r * p.t + p.n * p.t
    register = register_validation + register_update
    encrypt = p.n * p.m + p.n * p.t + p.n * p.t
    update = 0
    decrypt_c2_xid = int(p.t) * int(p.t)
